...
import { Clock3, Download, Edit, Eye, Plus, Receipt, RotateCcw, Send, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="min-w-0">
        <div className="inline-flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">
          <Receipt className="h-3.5 w-3.5 text-cyan-600 dark:text-cyan-300" />
          Expense Claims
        </div>
        ...
        actions={(
          <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/88">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
              <span className="font-medium text-slate-900 dark:text-slate-100">共 {remoteTotal} 条</span>
              ...
              <span className="text-slate-500 dark:text-slate-400">已打款 {paidCount}</span>
            </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  ...
                  <span className="ml-2 text-xs font-normal text-slate-500 dark:text-slate-400">
                    {hasActiveFilters ? `${currentStatusLabel} / ${currentCategoryLabel}` : '全部'}
                  </span>
                </div>
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  第 {searchParams.pageNum} / {totalPages} 页 · 共 {remoteTotal} 条
                </div>
              </div>
            </div>
              ...
              {formData.items?.map((item, index) => (
                ...
                >
                  ...
                  <div className="mt-3 space-y-2">
                    <Label className="text-xs font-medium text-slate-500 dark:text-slate-400">凭证链接</Label>
                    <Input
                      className="h-10"
                      type="text"
                      value={item.receiptUrl || ''}
                      onChange={(event) => updateItem(index, 'receiptUrl', event.target.value)}
                      placeholder="可选"
                    />
                  </div>
                </div>
        ...
        {detailLoading ? (
          ...
          <>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <DetailField label="申请人" value={renderDetailValue(detailClaim.userName)} />
              <DetailField label="所属部门" value={renderDetailValue(detailClaim.deptName)} />
              <DetailField label="报销类别" value={CATEGORY_LABELS[detailClaim.category] || detailClaim.category || '-'} />
              <DetailField label="总金额" value={formatAmount(detailClaim.totalAmount)} />
              <DetailField label="明细数量" value={`${detailClaim.items?.length || 0} 条`} />
              <DetailField label="流程实例" value={renderDetailValue(detailClaim.instanceId)} />
              <DetailField label="创建时间" value={renderDetailValue(detailClaim.createTime)} />
              <DetailField label="更新时间" value={renderDetailValue(detailClaim.updateTime)} />
            </div>
                          ...
                          <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">
                            {item.receiptUrl ? (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-cyan-700 underline decoration-dashed underline-offset-4 hover:text-cyan-800 dark:text-cyan-300 dark:hover:text-cyan-200"
                              >
                                查看凭证
                              </a>
                            ) : (
