          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              <tr key={item.licenseId}>
                ...
                <td>
                  <div className="font-medium text-cf-title">{item.licenseName}</div>
                  <div className="mt-1 text-xs text-cf-faint">{typeDict.getLabel(item.licenseType) || '-'}</div>
                </td>
                <td>
            ...
            <DialogPanel title="证照摘要">
              <dl className="admin-dialog-stack text-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <dd className="font-medium text-cf-title">{typeDict.getLabel(form.licenseType) || '-'}</dd>
                </div>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="admin-users-row-actions">
                        {item.status === 'DRAFT' && hasPermission('oa:license-renewal:edit') ? <button type="button" data-tooltip="编辑" aria-label="编辑" onClick={() => setRenewalForm({ ...item })}><Edit size={15} /></button> : null}
                        {item.status === 'DRAFT' && hasPermission('oa:license-renewal:submit') ? <button type="button" data-tooltip="提交" aria-label="提交" onClick={() => void submitRenewal(item.id)}><Send size={15} /></button> : null}
                        {item.status === 'PENDING' && hasPermission('oa:license-renewal:cancel') ? <button type="button" data-tooltip="取消" aria-label="取消" onClick={() => void cancelRenewal(item.id)}><XCircle size={15} /></button> : null}
                        {item.status && ['DRAFT', 'REJECTED', 'CANCELLED'].includes(item.status) && hasPermission('oa:license-renewal:remove') ? <button type="button" className="danger" data-tooltip="删除" aria-label="删除" onClick={() => void removeRenewal(item.id)}><Trash2 size={15} /></button> : null}
                      </div>
                    </div>
                  </div>
