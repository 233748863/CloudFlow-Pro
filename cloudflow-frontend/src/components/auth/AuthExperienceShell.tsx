    ...
    <ModalOverlay className="grid place-items-center bg-black/48 p-3 backdrop-blur-[2px] sm:p-4" closeOnClickOutside onClose={onClose}>
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="no-min-size absolute -right-3 -top-3 z-20 inline-flex h-9 w-9 items-center justify-center rounded-2xl border border-slate-200 bg-cf-surface-1 text-cf-faint shadow-sm transition-colors hover:text-cf-body dark:border-slate-700"
          aria-label="关闭验证弹层"
        >
          <X size={16} />
        </button>

        <div className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-cf-surface-1 p-3 shadow-[0_28px_70px_-30px_rgba(15,23,42,0.34)] dark:border-slate-700 dark:shadow-[0_32px_72px_-32px_rgba(2,6,23,0.58)]">
