...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  <div className={cn('empty-state py-12', className)}>
    ...
    {action ? (
      ...
      <div className="mt-8">
        {actionTo ? (
          <Link className="btn btn-primary rounded-md px-5 py-2.5" to={actionTo}>
            {actionIcon ? <Plus className="mr-2 h-4 w-4" /> : null}
            {actionText}
          </Link>
        ) : (
          <Button className="rounded-md px-5 py-2.5" onClick={onAction}>
            {actionIcon ? <Plus className="mr-2 h-4 w-4" /> : null}
            {actionText}
          </Button>
        )}
...
export const EmptyTasks: React.FC<{ onCreate?: () => void }> = ({ onCreate }) => (
  <EmptyState
    icon={<ClipboardList className="empty-state-icon" />}
    title="暂无任务"
...
export const EmptyWorkflows: React.FC<{ onCreate?: () => void }> = ({ onCreate }) => (
  <EmptyState
    icon={<SquareKanban className="empty-state-icon" />}
    title="暂无流程"
...
export const EmptyError: React.FC<{ onRetry?: () => void }> = ({ onRetry }) => (
  <EmptyState
    icon={<AlertTriangle className="empty-state-icon text-rose-500" />}
    title="加载失败"
...
export const EmptyPermission: React.FC = () => (
  <EmptyState
    icon={<Lock className="empty-state-icon text-amber-500" />}
    title="权限不足"
