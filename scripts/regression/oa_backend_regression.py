...
class OABackendRegression:
    ...
    def run(self):
        ...
        self.recorder.run("预检 Workflow 定义覆盖 OA 当前归属流程", self.precheck_workflow_definitions)
    ...
    def test_workplace_and_contact(self):
        summary = self.unwrap_ok(self.api("zhang").get("/oa/workplace/summary"), "查询工作台概览")
        ensure(summary.get("user") is not None, "工作台未返回用户信息")
        statistics = summary.get("statistics") or {}
        for key in ("pendingTasks", "todaySchedules", "unreadMessages"):
            ensure(key in statistics, f"工作台统计缺少字段: {key}")

        recent_tasks = self.unwrap_ok(self.api("zhang").get("/oa/workplace/recent-tasks", {"limit": 5}), "查询最近任务")
        ensure(isinstance(recent_tasks, list), "最近任务返回值不是列表")

        contact_page = self.unwrap_ok(
            self.api("zhang").get("/oa/contact/list", {"pageNum": 1, "pageSize": 20}),
            "查询通讯录列表",
        )
        contacts = normalize_page_rows(contact_page)
        ensure(len(contacts) > 0, "通讯录列表为空")
        dept_tree = self.unwrap_ok(self.api("zhang").get("/oa/contact/dept/tree"), "查询部门树")
        ensure(isinstance(dept_tree, list) and len(dept_tree) > 0, "部门树为空")

        first_contact = contacts[0]
        user_id = first_contact.get("user_id") or first_contact.get("userId")
        ensure(user_id is not None, f"通讯录记录缺少 user_id: {first_contact}")
        detail = self.unwrap_ok(self.api("zhang").get(f"/oa/contact/user/{user_id}"), "查询通讯录用户详情")
        ensure((detail.get("user_id") or detail.get("userId")) == user_id, "通讯录详情用户不匹配")

        return {
            "recentTaskCount": len(recent_tasks),
            "contactCount": len(contacts),
            "deptCount": len(dept_tree),
            "userId": user_id,
            "summaryKeys": sorted(summary.keys()),
        }

    def test_notices(self):
        notice_page = self.unwrap_ok(
            self.api("zhang").get("/oa/notice/list", {"pageNum": 1, "pageSize": 20}),
            "查询通知列表",
        )
        notices = normalize_page_rows(notice_page)
        unread_before = self.unwrap_ok(self.api("zhang").get("/oa/notice/unread-count"), "查询未读通知数")
        result = {
            "noticeCount": len(notices),
            "unreadBefore": unread_before,
        }
        if not notices:
            ensure(unread_before == 0, "通知列表为空但未读数不为 0")
            return result

        first_notice = notices[0]
        notice_id = first_notice.get("noticeId") or first_notice.get("notice_id") or first_notice.get("id")
        ensure(notice_id is not None, f"通知记录缺少主键: {first_notice}")
        detail = self.unwrap_ok(self.api("zhang").get(f"/oa/notice/{notice_id}"), "查询通知详情")
        self.unwrap_ok(self.api("zhang").post(f"/oa/notice/read/{notice_id}"), "标记通知已读")
        unread_after = self.unwrap_ok(self.api("zhang").get("/oa/notice/unread-count"), "查询已读后未读通知数")
        ensure(unread_after <= unread_before, "通知已读后未读数未下降或异常")
        result.update({
            "noticeId": notice_id,
            "unreadAfter": unread_after,
            "detailKeys": sorted(detail.keys()),
        })
        return result
    ...
    def test_work_tasks(self):
        ...
        ensure_equal(detail.get("description"), "OA 回归任务-已编辑", "任务描述未更新")
    ...
    def test_vehicle_and_convert(self):
        ...
        start_time = (dt.datetime.now() + dt.timedelta(days=1)).replace(hour=9, minute=0, second=0, microsecond=0)
        ...
        self.unwrap_ok(
            self.api("admin").get(
                "/oa/vehicle/expense/stats",
                {"startDate": format_date(dt.date.today().replace(day=1)), "endDate": format_date(dt.date.today())},
            ),
            "查询车辆费用统计",
        )
        self.unwrap_ok(
        ...
        self.unwrap_ok(
            self.api("admin").put(
                ...
                {"endMileage": 1288, "remark": "OA 回归还车"},
            ),
            "归还车辆",
        )
