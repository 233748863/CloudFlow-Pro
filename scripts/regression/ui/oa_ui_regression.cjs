...
const ROOT = path.join(PROJECT_ROOT, '.codex-temp');
...
const GATEWAY_BASE_URL = 'http://127.0.0.1:9000';
const API_HOST_MARKERS = ['127.0.0.1:9000', '127.0.0.1:9001', '127.0.0.1:9002', '127.0.0.1:9003'];
const EDGE_PATH = 'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe';
const REFRESH_TOKENS_SCRIPT = path.join(PROJECT_ROOT, 'scripts', 'regression', 'refresh_ui_tokens.py');
...
const REQUIRED_USERS = Array.from(new Set(PAGE_GROUPS.map((group) => group.user).filter(Boolean)));

function loadTokenMap() {
  return JSON.parse(fs.readFileSync(TOKENS_PATH, 'utf8'));
}

function refreshTokenMap() {
  const result = spawnSync(process.env.PYTHON || 'python', [REFRESH_TOKENS_SCRIPT], {
    cwd: PROJECT_ROOT,
    stdio: 'inherit',
  });
  if (result.error) {
...
async function validateToken(token) {
  ...
  try {
    const response = await fetch(`${GATEWAY_BASE_URL}/auth/info`, {
      headers: {
...
async function ensureTokenMap() {
  ...
  let invalidUsers = [];
  for (const username of REQUIRED_USERS) {
    const token = tokens?.users?.[username]?.token;
...
async function run() {
  const tokens = await ensureTokenMap();
