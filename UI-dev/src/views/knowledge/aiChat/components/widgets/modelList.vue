...
interface AiModel {
	id: string;
	name: string;
	defaultModel?: string; // 添加 defaultModel 字段
}

interface Props {
	modelType: ('Image' | 'Reason' | 'Chat')[];
}
...
// Load models when component is mounted or modelType changes
async function loadModels() {
	const res = await list({});
	// Filter models by modelType
	models.value = res.data.filter((model: any) => props.modelType.some((type) => type === model.modelType));
	// Get selected model from local storage or use first model as default
