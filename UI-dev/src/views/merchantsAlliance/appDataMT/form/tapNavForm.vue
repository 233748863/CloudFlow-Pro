  ...
  <div class="w-full flex flex-col h-full gap-4">
    ...
    >
      ...
      <template #header>
        <div class="flex flex-row items-center justify-between">
          ...
          <right-toolbar
            ...
            @queryTable="getPlatformNaviIndustry"
          />
        </div>
      </template>
      <el-table
      ...
      >
        ...
        <el-table-column label="行业分类" prop="targetId" width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <el-select
              ...
              filterable
              remote
              @change="(id: number) => handleIndustryChange(id, row)"
            >
              <el-option
...
// 分类导航菜单查询
async function getPlatformNaviIndustry() {
  ...
  try {
    ...
    if (resp.code === 0) {
      ...
      state.industryList = resp.data.records.map((item:any) => ({
        id: item.targetId,
        name: item.name,
        enable: true,
      }))
...
// 行业分类选择器改变事件
const handleIndustryChange = (id: number, row: PlatformNaviMenuData) => {
  const targetId = id.toString()
  row.name = state.industryList.find((item) => item.id === targetId)?.name || ''
