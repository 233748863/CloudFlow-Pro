...
export const adminPageConfigs: AdminPageConfig[] = [
  ...
  },
  {
    ...
    title: '印章台账',
    eyebrow: 'Seal Ledger',
    description: '维护印章编码、编号、签发方和到期日期',
    icon: Stamp,
    basePath: '/oa/seal',
    idKey: 'sealId',
    primaryKey: 'sealName',
    searchPlaceholder: '印章名称/编号/保管人',
    fields: [
      ...
      text('sealNo', '印章编号', { filter: true }),
      text('issuer', '签发机关'),
      date('issueDate', '签发日期'),
      date('expireDate', '到期日期'),
      number('keeperId', '保管人ID', { table: false }),
      text('keeperName', '保管人', { filter: true }),
      text('location', '存放位置'),
      text('attachmentUrl', '附件', { formatter: (value) => String(value || '').split(',').filter(Boolean).length ? `${String(value || '').split(',').filter(Boolean).length} 个` : '-' }),
