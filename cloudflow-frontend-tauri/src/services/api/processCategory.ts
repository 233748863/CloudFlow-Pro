...
export const processCategoryApi = {
  /** 查询分类树(仅正常状态) */
  tree: () => request.get<ProcessCategory[]>('/workflow/category/tree'),

  /** 查询所有分类(含停用) */
  list: () => request.get<ProcessCategory[]>('/workflow/category/list'),

  /** 查询分类详情 */
  getInfo: (categoryId: number) =>
    request.get<ProcessCategory>(`/workflow/category/${categoryId}`),
