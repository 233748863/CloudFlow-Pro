...
6. ⏳ **批量编辑**
   - 支持批量修改流程分类
   - 支持批量添加标签
   - 注：批量编辑功能建议在管理后台实现，当前用户端已完成核心展示和筛选功能
...
**建议方案**:
批量编辑功能更适合在管理后台实现，原因如下：

1. **权限控制**: 批量修改流程定义需要管理员权限
2. **操作复杂度**: 需要选择多个流程、确认修改、处理冲突
3. **用户场景**: 普通用户主要需求是发起流程，不需要批量编辑
4. **UI 空间**: 当前页面聚焦于流程发起，添加批量编辑会使界面复杂

**实现建议**:
- 在管理后台创建"流程管理"页面
- 提供流程列表（带复选框）
- 支持批量操作：修改分类、添加/删除标签、归档流程
- 使用确认对话框防止误操作
