...
- 权限码统一改名为 `{module}:{resource}:{action}`，模块前缀只允许 `system`、`oa`、`crm`、`hr`、`workflow` 五类；本次采用一次切换，不保留旧权限码兼容层；React 和 Vue 两套前端同步完成。
...
- 前端：React 与 Vue 同步替换权限常量、路由守卫、菜单显隐、按钮 `permissionKey`、认证 store 的权限判断；不保留旧权限码别名。
...
- 当前前端显隐面：React 已有 78 处路由守卫、128 处动作按钮权限点、72 处 `hasPermission()` 调用；Vue 侧涉及 auth store、router、菜单加载与页面路由元数据，需全量同步。
...
- 一次切换验收：执行顺序固定为“DB 迁移 → 后端发布 → React/Vue 发布 → 全链路冒烟”；回滚演练必须覆盖 SQL 回滚和应用版本回退。
