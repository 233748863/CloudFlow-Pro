...
package cn.joywon.poco.auth.endpoint;
...
import cn.joywon.poco.admin.api.entity.SysOauthClientDetails;
import cn.joywon.poco.admin.api.entity.SysTenant;
import cn.joywon.poco.admin.api.feign.RemoteClientDetailsService;
import cn.joywon.poco.admin.api.feign.RemoteTenantService;
import cn.joywon.poco.admin.api.vo.TokenVO;
...
import cn.joywon.poco.common.security.util.OAuth2ErrorCodesExpand;
import cn.joywon.poco.common.security.util.OAuthClientException;
...
public class PocoTokenEndpoint {
    ...
    public R<Page<TokenVO>> tokenList(@RequestBody Map<String, Object> params) {
        ...
        List<TokenVO> tokenVoList = redisTemplate.opsForValue().multiGet(pages).stream().map(obj -> {
            ...
            PocoUser pocoUser = (PocoUser) authentication.getPrincipal();
            tokenVo.setUserId(pocoUser.getId());
            return tokenVo;
        }).filter(tokenVo -> {
            // 根据用户名过滤
